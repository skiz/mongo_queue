...
which can be adjusted to your taste.
    ...
    db = Mongo::Connection.whatever...

    options = {
      :database   => 'mongo_queue', 
      :collection => 'mongo_queue',
      :timeout    => 30, # number of seconds that an item will remain locked
      :attempts   => 3   # number of times attempts to lock and complete the item before ignoring
    }
    queue = MongoQueue.new(db, options)

=== Inserting Jobs
A job is inserted as a hash in to the document store, but you will want to avoid using the protected keys which
enable MongoQueue to operate effectively as a queue system. Avoid using the following fields in your queue messages;
* locked_at
* locked_by
* attempts
* timeout
